...
namespace Levels
{
	...
	Level1::Level1(Map map_) : Level("Level1"),
		...
		playWinSound(false),
		map(map_),
		backgroundLayers{ nullptr }, shadow(nullptr),
	...
	void Level1::Load()
	{
		...
		resourceManager.GetSpriteSource("ArtBackgroundLayer1.png");
		resourceManager.GetSpriteSource("ArtBackgroundLayer2.png");
		resourceManager.GetSpriteSource("ArtBackgroundLayer3.png");
	...
	void Level1::Initialize()
	{
		...
		backgroundLayers[0] = new GameObject(*objectManager.GetArchetypeByName("BackgroundImage"));
		backgroundLayers[0]->GetComponent<Sprite>()->SetSpriteSource(resourceManager.GetSpriteSource("ArtBackgroundLayer1.png"));
		objectManager.AddObject(*backgroundLayers[0]);
		backgroundLayers[1] = new GameObject(*objectManager.GetArchetypeByName("BackgroundImage"));
		backgroundLayers[1]->GetComponent<Sprite>()->SetSpriteSource(resourceManager.GetSpriteSource("ArtBackgroundLayer2.png"));
		objectManager.AddObject(*backgroundLayers[1]);
		backgroundLayers[2] = new GameObject(*objectManager.GetArchetypeByName("BackgroundImage"));
		backgroundLayers[2]->GetComponent<Sprite>()->SetSpriteSource(resourceManager.GetSpriteSource("ArtBackgroundLayer3.png"));
		objectManager.AddObject(*backgroundLayers[2]);
		...
		if (dataStaticMap != nullptr && dataRedMap != nullptr && dataBlueMap != nullptr)
		{
			...
			case Map::Cavern:
			{
				float staticSpikes[38] = {
					11.0f, 12.0f, 12.0f, 12.0f, 11.0f, 13.0f, 12.0f, 13.0f, 2.0f, 22.0f, 3.0f, 22.0f, 18.0f, 25.0f, 19.0f, 25.0f, 12.0f, 46.0f, 13.0f, 46.0f, 14.0f, 46.0f, 15.0f, 46.0f, 16.0f, 46.0f, 17.0f, 46.0f, 18.0f, 46.0f, 19.0f, 46.0f,
					20.0f, 46.0f, 21.0f, 46.0f, 22.0f, 46.0f
				};
				float redSpikes[22] = {
				...
				float blueSpikes[14] = {
					4.0f, 4.0f, 5.0f, 4.0f, 11.0f, 4.0f, 12.0f, 4.0f, 8.0f, 22.0f, 9.0f, 22.0f, 10.0f, 22.0f
				};
				float abilities[24] = {
				...
				AddStaticSpikes(staticSpikes, 19);
				AddRedSpikes(redSpikes, 11, redDimension);
				AddBlueSpikes(blueSpikes, 7, blueDimension);
				AddAbilities(abilities, 12);
			...
			risingGears->GetComponent<Behaviors::RisingGears>()->SetStartOffset(risingGears->GetComponent<Behaviors::RisingGears>()->GetStartOffset() + gearDelay);
	...
	void Level1::Update(float dt)
	{
		...
		GameObject* tilemap = objectManager.GetObjectByName("Tilemap");
		SpriteTilemap* spriteTilemap = tilemap->GetComponent<SpriteTilemap>();
		Vector2D topLeft = spriteTilemap->TileToWorld(Vector2D(0.0f, 0.0f));
		Vector2D bottomRight = spriteTilemap->TileToWorld(Vector2D(static_cast<float>(dataStaticMap->GetWidth()), static_cast<float>(dataStaticMap->GetHeight())));
		Vector2D center = (topLeft + bottomRight) / 2.0f;
		...
		backgroundLayers[1]->GetComponent<Transform>()->SetScale(Interpolate(Vector2D(cameraSize * 4.0f, cameraSize * 3.0f), Vector2D(60.0f, 45.0f), 0.9f));
		backgroundLayers[2]->GetComponent<Transform>()->SetTranslation(Interpolate(Vector2D(cameraPos.x, bottomRight.y + 10.0f), Vector2D(center.x, bottomRight.y + 5.0f), 0.75f));
		backgroundLayers[2]->GetComponent<Transform>()->SetScale(Vector2D(20.0f, 40.0f));
